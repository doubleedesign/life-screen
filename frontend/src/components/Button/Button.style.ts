...
export const ButtonWrapper = styled.button<{$appearance: ThemeColour, $isCollapsed?: boolean, $isActive?: boolean}>`
	padding: ${props => props.theme.spacing.sm} ${props => props.$isCollapsed ? props.theme.spacing.sm : props.theme.spacing.md};
	background: ${props => {
		if(props.$isActive) {
	...
	svg {
		margin-right: ${props => props.$isCollapsed ? 0 : props.theme.spacing.xs};
	}
